 ...
 * 초안이 기대고 있는 근거 한 조각.
 *
 * 원천 기록(하루 기록)과 성격이 다르다. 기록은 **사실**이 어디서 왔는지이고,
 * 근거는 **어떤 말로 어떻게 서술할지**의 기준이다. 그래서 화면에서도 따로 둔다 —
 * 밑줄은 기록에만 긋고, 근거는 문서 단위로 펼쳐 본다.
 */
export type Citation = {
  /** 사람이 읽는 문서 이름 — "누리과정 (3~5세)" 등 */
  doc: string;
  /** 문서 안 위치 — "Ⅰ. 신체운동·건강 › 나 잘하죠?" */
  where: string;
  page: number | null;
  /** 프롬프트에 실린 문단 그대로 */
  text: string;
  /** 발달영역·평가지표 — 어느 칸을 위해 찾은 근거인가 */
  slot: string | null;
};
...
export type DocumentDraft = {
  ...
  /**
   * 이 초안이 실제로 참고한 근거(FN-004). 검색을 안 하는 타입(알림장)은 빈 배열.
   *
   * 생성 시점에 서버가 박제한 값이라 다시 열어도 그때 본 근거가 그대로 나온다.
   */
  citations: Citation[];
  /**
